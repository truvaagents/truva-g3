...
- **Go's Design Philosophy** ("Accept interfaces, return structs"): "The bigger the interface, the weaker the abstraction." Rob Pike: "Simplicity is complicated — it requires work to achieve." Go's 1.0 Compatibility Guarantee keeps Go 1 code source-compatible across later Go 1.x releases — stability comes from narrow, focused APIs. **Lesson**: Small interfaces that compose > large interfaces that prescribe. ([source](https://dave.cheney.net/2016/08/20/solid-go-design))
