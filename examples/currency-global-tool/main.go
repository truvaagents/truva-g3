...
func main() {
	...
	framework, err := core.NewFramework(tool,
		// Core configuration
		core.WithName("currency-global-tool"),
		core.WithPort(port),
		...
		core.WithMiddleware(telemetry.TracingMiddlewareWithConfig("currency-global-tool", &telemetry.TracingMiddlewareConfig{
	...
	tool.Logger.Info("Currency Global Tool Service Starting", map[string]interface{}{
		"service":    "currency-global-tool",
		"port":       port,
		"telemetry":  "enabled",
		"api":        "CurrencyBeacon",
		"currencies": "170+",
...
func validateConfig() error {
	...
	// CURRENCYBEACON_API_KEY is required for currency data
	apiKey := os.Getenv("CURRENCYBEACON_API_KEY")
	if apiKey == "" {
		log.Println("Warning: CURRENCYBEACON_API_KEY not set - API calls will fail")
	}
