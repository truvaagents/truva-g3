#!/bin/bash
# Currency Global Tool Setup Script
# Provides commands for building, running, and deploying the global currency conversion tool
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Currency Global Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Currency Global Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o currency-global-tool .

    print_success "Build completed: currency-global-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Currency Global Tool"
    ...
    if [ -z "$CURRENCYBEACON_API_KEY" ] || [ "$CURRENCYBEACON_API_KEY" = "your-currencybeacon-api-key-here" ]; then
        print_error "CURRENCYBEACON_API_KEY is required"
        print_info "Get a free key at: https://currencybeacon.com/register"
        print_info "Then set it in .env file"
        exit 1
    fi
    ...
    print_info "Starting currency-global-tool on port $PORT..."
    print_info "Redis URL: $REDIS_URL"
    print_info "API: CurrencyBeacon (170+ currencies)"
    echo ""
...
setup_api_keys() {
    ...
    truvag3_create_tool_secret "currency-global-tool-secrets" "$NAMESPACE" "CURRENCYBEACON_API_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "currency-global-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/currency-global-tool-service 8346:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8346/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8346/api/capabilities | grep -q "capabilities"; then
        print_success "Capabilities endpoint working"
    else
        print_error "Capabilities endpoint not responding"
    fi

    # Test currency conversion (USD to INR - global currency)
    echo ""
    print_info "Testing currency conversion (USD to INR - global currency)..."
    curl -s -X POST http://localhost:8346/api/capabilities/convert_currency \
        -H "Content-Type: application/json" \
        -d '{"from": "USD", "to": "INR", "amount": 100}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    echo ""
    print_info "Testing currency conversion (EUR to CNY - non-ECB pair)..."
    curl -s -X POST http://localhost:8346/api/capabilities/convert_currency \
        -H "Content-Type: application/json" \
        -d '{"from": "EUR", "to": "CNY", "amount": 500}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    echo ""
    print_info "Testing exchange rates (base: EUR)..."
    curl -s -X POST http://localhost:8346/api/capabilities/get_exchange_rates \
        -H "Content-Type: application/json" \
        -d '{"base": "EUR", "currencies": ["USD", "GBP", "INR", "CNY", "NGN"]}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "currency-global-tool-service" 8346 80
}

# Port forward for tool and monitoring
cmd_forward_all() {
    truvag3_forward_all \
        "currency-global-tool-service:8346:80" \
        "grafana:3000:80" \
...
cmd_status() {
    ...
    echo "Currency Global Tool Pod:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "Currency Global Tool Service:"
    kubectl get svc -n $NAMESPACE currency-global-tool-service
    echo ""
...
# Clean up tool only
cmd_clean() {
    print_header "Cleaning Up Currency Global Tool"

    print_info "Removing tool deployment..."
    kubectl delete -f k8-deployment.yaml --ignore-not-found
    print_success "Currency global tool cleanup complete"
}
...
# Show help
cmd_help() {
    echo "Currency Global Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "Local Development Commands:"
    echo "  build         Build the currency-global-tool binary"
    ...
    echo "  PORT                     HTTP server port (default: 8346)"
    echo "  CURRENCYBEACON_API_KEY   CurrencyBeacon API key for 170+ currencies"
    echo "                           Get a FREE key: https://currencybeacon.com/register"
    echo ""
    echo "Examples:"
    ...
    echo "  REDIS_URL=redis://localhost:6379 CURRENCYBEACON_API_KEY=your-key ./setup.sh run"
}
