	...
	d.RegisterCapability(core.Capability{
		...
		OutputSummary: &core.SchemaSummary{
			RequiredFields: []core.FieldHint{
				...
				{Name: "exit_code", Type: "number", Description: "Exit code: 0 = success, non-zero = failure. kubectl's own codes are passed through; tool-internal codes are 124 (timed out), 125 (cancelled by caller), and 75 (busy — no execution slot was free, the command did not run; safe to retry). 75/124/125 are NOT cluster or permission errors.", Example: "0"},
