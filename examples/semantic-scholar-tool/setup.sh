...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/semantic-scholar-service 8370:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3
...
cmd_forward() {
    truvag3_forward "semantic-scholar-service" 8370 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "semantic-scholar-service:8370:80" \
        "grafana:3000:80" \
