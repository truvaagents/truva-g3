#!/bin/bash
# Semantic Scholar Tool Setup Script
# Provides commands for building, running, and deploying the semantic scholar tool
...
APP_NAME="semantic-scholar-tool"
PORT=${PORT:-8370}
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Semantic Scholar Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Semantic Scholar Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o semantic-scholar-tool .

    print_success "Build completed: semantic-scholar-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Semantic Scholar Tool"
    ...
    print_info "Starting semantic-scholar-tool on port $PORT..."
...
setup_config() {
    truvag3_create_configmap "semantic-scholar-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    truvag3_create_tool_secret "s2-tool-secrets" "$NAMESPACE" "S2_API_KEY"
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy semantic scholar tool"
    ...
    echo "  Semantic Scholar Tool:  http://localhost:8370"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/semantic-scholar-tool-service 8370:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8370/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8370/api/capabilities | grep -q "capabilities"; then
    ...
    # Test search papers
    echo ""
    print_info "Testing search papers..."
    curl -s -X POST http://localhost:8370/api/capabilities/search_papers \
        -H "Content-Type: application/json" \
        -d '{"query": "transformer architecture"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "semantic-scholar-tool-service" 8370 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "semantic-scholar-tool-service:8370:80" \
        "grafana:3000:80" \
...
# Show help
cmd_help() {
    echo "Semantic Scholar Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the semantic scholar tool service only"
    ...
    echo "  clean         Remove semantic scholar tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8370)"
    echo "  S2_API_KEY        Semantic Scholar API key (strongly recommended)"
    echo "                    Get a key: https://www.semanticscholar.org/product/api#api-key-form"
    echo ""
    echo "Configuration:"
