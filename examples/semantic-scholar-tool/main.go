...
func main() {
	...
	tool := NewSemanticScholarTool()

	// Initialize telemetry AFTER tool creation
	initTelemetry("semantic-scholar-tool")
	defer func() {
	...
	port := 8370 // Semantic Scholar tool default port
	...
	framework, err := core.NewFramework(tool,
		core.WithName("semantic-scholar-tool"),
		core.WithPort(port),
		...
		core.WithMiddleware(telemetry.TracingMiddlewareWithConfig("semantic-scholar-tool",
	...
	tool.Logger.Info("Semantic Scholar Tool Service Starting", map[string]interface{}{
		"operation": "startup",
		"service":   "semantic-scholar-tool",
		"port":      port,
		"telemetry": "enabled",
	})
...
func validateConfig() error {
	...
	// S2_API_KEY is optional but strongly recommended
	apiKey := os.Getenv("S2_API_KEY")
	if apiKey == "" {
		fmt.Fprintln(os.Stderr, "Warning: S2_API_KEY not set - requests will use shared rate limit pool (expect 429s)")
	}
