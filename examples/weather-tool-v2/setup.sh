...
setup_config() {
    truvag3_create_configmap "weather-tool-v2-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_forward() {
    truvag3_forward "weather-tool-v2-service" 8339 80
}

# Port forward for tool and monitoring
cmd_forward_all() {
    truvag3_forward_all \
        "weather-tool-v2-service:8339:80" \
        "grafana:3000:80" \
...
cmd_help() {
    ...
    echo "  PORT          HTTP server port (default: 8339)"
    echo ""
    ...
    echo "  curl -X POST http://localhost:8339/api/capabilities/get_current_weather \\"
    echo "    -H 'Content-Type: application/json' \\"
    echo "    -d '{\"lat\": 40.7128, \"lon\": -74.0060}'"
    echo ""
    echo "  # 7-day forecast (Tokyo)"
    echo "  curl -X POST http://localhost:8339/api/capabilities/get_weather_forecast \\"
    echo "    -H 'Content-Type: application/json' \\"
