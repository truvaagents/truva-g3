...
The arXiv tool requires Redis for service discovery. If you haven't already set up infrastructure:
...
# Build Docker image
docker build -t arxiv-tool:latest .

# Load into Kind
kind load docker-image arxiv-tool:latest

# Deploy to Kubernetes
kubectl apply -f k8-deployment.yaml

# Verify deployment
kubectl get pods -n truvag3-examples -l app=arxiv-tool
```
...
kubectl port-forward -n truvag3-examples svc/arxiv-service 8369:80
...
```bash
# Check logs
kubectl logs -n truvag3-examples -l app=arxiv-tool | grep -i "api\|error"
...
```bash
# View tool logs
kubectl logs -n truvag3-examples -l app=arxiv-tool
