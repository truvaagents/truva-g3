#!/bin/bash
# arXiv Tool Setup Script
# Provides commands for building, running, and deploying the arXiv tool
...
# Build the tool
cmd_build() {
    print_header "Building arXiv Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o arxiv-tool .

    print_success "Build completed: arxiv-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running arXiv Tool"
    ...
    print_info "Starting arxiv-tool on port $PORT..."
...
setup_config() {
    truvag3_create_configmap "arxiv-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    # NO truvag3_create_tool_secret call -- arXiv has no API key
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  arXiv Tool:  http://localhost:8369"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/arxiv-tool-service 8369:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8369/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8369/api/capabilities | grep -q "capabilities"; then
    ...
    curl -s -X POST http://localhost:8369/api/capabilities/search_papers \
        -H "Content-Type: application/json" \
        -d '{"query": "attention is all you need"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "arxiv-tool-service" 8369 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "arxiv-tool-service:8369:80" \
        "grafana:3000:80" \
...
cmd_help() {
    ...
    echo "  forward       Port forward the arXiv tool service only"
    ...
    echo "  clean         Remove arXiv tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8369)"
    echo ""
    echo "Note: arXiv API requires NO authentication (no API key needed)."
    echo "Documentation: https://info.arxiv.org/help/api/user-manual.html"
    echo ""
    echo "Configuration:"
