...
setup_config() {
    truvag3_create_configmap "currency-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/currency-tool-service 8334:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8334/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8334/api/capabilities | grep -q "capabilities"; then
    ...
    curl -s -X POST http://localhost:8334/api/capabilities/convert_currency \
...
cmd_forward() {
    truvag3_forward "currency-tool-service" 8334 80
}

# Port forward for tool and monitoring
cmd_forward_all() {
    truvag3_forward_all \
        "currency-tool-service:8334:80" \
        "grafana:3000:80" \
...
cmd_help() {
    ...
    echo "  PORT              HTTP server port (default: 8334)"
