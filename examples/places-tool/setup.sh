...
APP_NAME="places-tool"
PORT=${PORT:-8344}
REDIS_URL=${REDIS_URL:-redis://localhost:6379}
EXAMPLES_DIR="$(dirname "$SCRIPT_DIR")"
source "$EXAMPLES_DIR/k8-deployment/setup-env-lib.sh"

print_header() { echo -e "${BLUE}================================================${NC}"; echo -e "${BLUE}  Places Tool - $1${NC}"; echo -e "${BLUE}================================================${NC}"; }
...
setup_api_keys() {
    ...
    truvag3_create_tool_secret "places-tool-secrets" "$NAMESPACE" "FOURSQUARE_API_KEY" "GEOAPIFY_API_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "places-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env" "PLACES_DEFAULT_PROVIDER" "OTEL_SERVICE_NAME"
}
...
cmd_rollout() { load_env; setup_api_keys; setup_config; kubectl apply -f k8-deployment.yaml; kubectl rollout restart deployment/$APP_NAME -n $NAMESPACE; }
cmd_forward_all() {
    truvag3_forward_all \
        "places-tool-service:8344:80" \
        "grafana:3000:80" \
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/places-tool-service 8344:80 >/dev/null 2>&1 &
    PF_PID=$!; sleep 3
    curl -s http://localhost:8344/health | grep -q "healthy" && print_success "Health OK" || print_error "Health failed"
    print_info "Testing place search..."
    curl -s -X POST http://localhost:8344/api/capabilities/search_places \
        -H "Content-Type: application/json" \
        -d '{"query":"sushi restaurants","lat":35.6762,"lon":139.6503}' | jq . 2>/dev/null || echo "(install jq)"
    kill $PF_PID 2>/dev/null || true
}

cmd_forward() {
    truvag3_forward "places-tool-service" 8344 80
}
...
cmd_rebuild() { load_env; DOCKER_NO_CACHE=true cmd_docker_build
    if command -v kind &> /dev/null; then kind load docker-image $APP_NAME:latest --name "$CLUSTER_NAME"; fi
    kubectl create namespace $NAMESPACE --dry-run=client -o yaml | kubectl apply -f -; setup_api_keys; setup_config; kubectl apply -f k8-deployment.yaml
...
cmd_help() {
    echo "Places Tool Setup Script"; echo "Usage: ./setup.sh <command>"
    ...
    echo "Env: REDIS_URL PORT FOURSQUARE_API_KEY GEOAPIFY_API_KEY PLACES_DEFAULT_PROVIDER"
}
