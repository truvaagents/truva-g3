#!/bin/bash
# PubMed Tool Setup Script
# Provides commands for building, running, and deploying the PubMed tool
...
# Build the tool
cmd_build() {
    print_header "Building PubMed Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o pubmed-tool .

    print_success "Build completed: pubmed-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running PubMed Tool"
    ...
    print_info "Starting pubmed-tool on port $PORT..."
...
setup_config() {
    truvag3_create_configmap "pubmed-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    truvag3_create_tool_secret "pubmed-tool-secrets" "$NAMESPACE" "NCBI_API_KEY"
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  PubMed Tool: http://localhost:8375"
...
cmd_test() {
    ...
    if curl -s http://localhost:8375/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8375/api/capabilities | grep -q "capabilities"; then
    ...
    echo ""
    print_info "Testing search articles..."
    curl -s -X POST http://localhost:8375/api/capabilities/search_articles \
        -H "Content-Type: application/json" \
        -d '{"query": "cancer immunotherapy"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_help() {
    ...
    echo "  forward       Port forward the PubMed tool service only"
    ...
    echo "  clean         Remove PubMed tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8375)"
    echo "  NCBI_API_KEY      NCBI API key (optional, increases rate from 3 to 10 req/sec)"
