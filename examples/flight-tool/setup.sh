#!/bin/bash
# Flight Tool Setup Script
# Provides commands for building, running, and deploying the flight tool
...
# Build the tool
cmd_build() {
    print_header "Building Flight Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o flight-tool .

    print_success "Build completed: flight-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Flight Tool"
    ...
    print_info "Starting flight-tool on port $PORT..."
...
setup_api_keys() {
    ...
    truvag3_create_tool_secret "flight-tool-secrets" "$NAMESPACE" "TRAVELPAYOUTS_TOKEN"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "flight-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env" "OTEL_SERVICE_NAME"
}
...
cmd_test() {
    ...
    if curl -s http://localhost:8342/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8342/api/capabilities | grep -q "capabilities"; then
    ...
    # Test airport search
    echo ""
    print_info "Testing airport search..."
    curl -s -X POST http://localhost:8342/api/capabilities/search_airports \
        -H "Content-Type: application/json" \
        -d '{"keyword":"Tokyo"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Test flight search
    echo ""
    print_info "Testing flight search..."
    curl -s -X POST http://localhost:8342/api/capabilities/search_flights \
        -H "Content-Type: application/json" \
        -d '{"origin":"JFK","destination":"NRT","departure_date":"2026-04-15","adults":1}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_status() {
    ...
    echo "Flight Tool Pod:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "Flight Tool Service:"
...
cmd_clean() {
    ...
    print_info "Removing flight tool deployment..."
...
cmd_help() {
    ...
    echo "  PORT                   HTTP server port (default: 8342)"
    echo "  TRAVELPAYOUTS_TOKEN    Travelpayouts Data API token (flight prices)"
    echo ""
    echo "Examples:"
