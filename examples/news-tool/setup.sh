...
setup_api_keys() {
    truvag3_create_tool_secret "news-tool-secrets" "$NAMESPACE" "GNEWS_API_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "news-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/${APP_NAME}-service 8337:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8337/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8337/api/capabilities | grep -q "capabilities"; then
    ...
    curl -s -X POST http://localhost:8337/api/capabilities/search_news \
...
cmd_forward() {
    truvag3_forward "news-tool-service" 8337 80
}

# Port forward for tool and monitoring
cmd_forward_all() {
    truvag3_forward_all \
        "news-tool-service:8337:80" \
        "grafana:3000:80" \
...
cmd_help() {
    ...
    echo "  PORT              HTTP server port (default: 8337)"
