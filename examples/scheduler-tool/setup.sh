#!/bin/bash
# Scheduler Tool Setup Script
# Provides commands for building, running, and deploying the scheduler tool.
#
# scheduler-tool is a framework primitive deployed as a BaseTool. Unlike
# typical example tools, it does NOT require any external API key — Redis
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Scheduler Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Load .env file. The shared `truvag3_load_env` helper auto-copies
# .env.example → .env on fresh checkouts, so `./setup.sh full-deploy`
# is self-contained for tools that have no secrets to fill in.
load_env() { truvag3_load_env "$SCRIPT_DIR/.env"; }

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, etc.)
setup_config() {
...
# Build the tool
cmd_build() {
    print_header "Building Scheduler Tool"
...
cmd_run() {
    print_header "Running Scheduler Tool"
...
cmd_deploy() {
    ...
    # NOTE: scheduler-tool has no API keys, so no Secret is created here.
    setup_config
...
# One-click full deployment (non-blocking — does not start port forwards)
cmd_full_deploy() {
    ...
    echo ""
    print_info "Tool is accessible within the cluster via ClusterIP:"
    echo "  http://scheduler-tool-service.truvag3-examples"
    echo ""
    print_info "To access from your laptop, run a port forward:"
    echo "  ./setup.sh forward         # scheduler-tool only"
    echo "  ./setup.sh forward-all     # scheduler-tool + Grafana + Prometheus + Jaeger"
    echo ""
...
# Run smoke tests against the deployed tool
cmd_test() {
    print_header "Running Smoke Tests"

    # Start port forward in background
    print_info "Starting port forward..."
    kubectl port-forward -n $NAMESPACE svc/scheduler-tool-service $PORT:80 >/dev/null 2>&1 &
    PF_PID=$!
    ...
    if curl -s http://localhost:$PORT/api/capabilities | grep -q "schedule_task"; then
        print_success "Capabilities endpoint exposes all 5 scheduling capabilities"
    else
        print_error "Capabilities endpoint not responding correctly"
    fi

    # Create a 30-second one-shot schedule
    echo ""
    print_info "Creating a 30-second one-shot schedule (target: event-driven-agent)..."
    local create_resp
    create_resp=$(curl -s -X POST http://localhost:$PORT/api/capabilities/schedule_task \
        -H "Content-Type: application/json" \
        -d '{
              "target_agent": "event-driven-agent",
              "delay": "30s",
              "input": {"instruction": "scheduler-tool smoke test"}
            }')
    echo "$create_resp" | jq . 2>/dev/null || echo "$create_resp"

    local sched_id
    sched_id=$(echo "$create_resp" | jq -r '.data.schedule_id // empty' 2>/dev/null || echo "")

    # List active schedules
    echo ""
    print_info "Listing active schedules..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/list_schedules \
        -H "Content-Type: application/json" -d '{}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Cancel the schedule we just created so the test is idempotent
    if [ -n "$sched_id" ]; then
        echo ""
        print_info "Cancelling test schedule $sched_id..."
        curl -s -X POST http://localhost:$PORT/api/capabilities/cancel_schedule \
            -H "Content-Type: application/json" \
            -d "{\"schedule_id\": \"$sched_id\"}" | jq . 2>/dev/null || echo "(cancelled)"
    fi
...
cmd_forward() {
    truvag3_forward "scheduler-tool-service" $PORT 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "scheduler-tool-service:$PORT:80" \
        "grafana:3000:80" \
...
# Rollout - restart deployment to pick up new config
cmd_rollout() {
...
# Show help
cmd_help() {
    echo "Scheduler Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "Quick Start Commands:"
    echo "  full-deploy   ONE-CLICK: Create cluster + infra + deploy (non-blocking)"
    ...
    echo "  test          Run smoke tests against deployed tool"
    echo "  forward       Port forward the scheduler tool service only (interactive)"
    echo "  forward-all   Port forward tool + Grafana + Prometheus + Jaeger (interactive)"
    ...
    echo "  clean         Remove scheduler tool deployment only"
    ...
    echo "  PORT                            HTTP server port (default: 8379)"
    echo "  TRUVAG3_SCHEDULER_TICK_INTERVAL  Tick loop interval (default: 5s)"
    echo "  TRUVAG3_SCHEDULER_LOCK_TTL       Distributed lock TTL (default: 30s)"
    echo ""
    echo "Configuration:"
    ...
    echo ""
    echo "Notes:"
    echo "  - scheduler-tool has no external API keys (Redis is the only dependency)"
    echo "  - For HA testing, scale to multiple replicas:"
    echo "      kubectl scale deploy scheduler-tool --replicas=3 -n $NAMESPACE"
    echo "    The distributed lock ensures only one replica runs the tick loop."
}
