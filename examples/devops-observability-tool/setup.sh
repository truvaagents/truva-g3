#!/bin/bash
# DevOps Observability Tool Setup Script
# Provides commands for building, running, and deploying the devops observability tool
...
APP_NAME="devops-observability-tool"
PORT=${PORT:-8378}
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  DevOps Observability Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building DevOps Observability Tool"
...
cmd_run() {
    print_header "Running DevOps Observability Tool"
...
cmd_deploy() {
    ...
    # Create ConfigMap from .env (no secrets needed — Loki has no auth)
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy devops observability tool"
    ...
    echo "  DevOps Observability Tool:  http://localhost:$PORT"
...
cmd_test() {
    ...
    echo ""
    print_info "Testing get_labels..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/get_labels \
        -H "Content-Type: application/json" \
        -d '{"since": "24h"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    echo ""
    print_info "Testing get_label_values (service_name)..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/get_label_values \
        -H "Content-Type: application/json" \
        -d '{"label": "service_name"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    echo ""
    print_info "Testing query_logs (last 30m, limit 3)..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/query_logs \
        -H "Content-Type: application/json" \
        -d '{"query": "{service_name=~\".+\"}", "limit": 3, "since": "30m"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    echo ""
    print_info "Testing get_detected_fields (devops-chat-agent)..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/get_detected_fields \
        -H "Content-Type: application/json" \
        -d '{"query": "{service_name=\"devops-chat-agent\"}", "limit": 10}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "${APP_NAME}-service" "$PORT" 80
}
...
# Show help
cmd_help() {
    echo "DevOps Observability Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the devops observability tool service only"
    ...
    echo "  clean         Remove devops observability tool deployment only"
    ...
    echo "  PORT          HTTP server port (default: 8378)"
    echo "  LOKI_URL      Loki base URL (set in .env, e.g. http://loki.truvag3-examples:3100)"
    echo "  JAEGER_URL    Jaeger query base URL (optional, set in .env)"
    echo ""
    echo "Configuration:"
