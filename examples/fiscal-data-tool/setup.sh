#!/bin/bash
# Fiscal Data Tool Setup Script
# Provides commands for building, running, and deploying the fiscal-data-tool
# NOTE: No API key required - Treasury Fiscal Data API is completely open
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Fiscal Data Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Fiscal Data Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o fiscal-data-tool .

    print_success "Build completed: fiscal-data-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Fiscal Data Tool"
    ...
    print_info "Starting fiscal-data-tool on port $PORT..."
    print_info "Redis URL: $REDIS_URL"
    print_info "No API key required - Treasury Fiscal Data API is open"
    echo ""
...
# NOTE: No API key secret needed - Treasury Fiscal Data API is completely open
setup_config() {
    truvag3_create_configmap "fiscal-data-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    # Setup config (no secrets needed - Treasury API is open)
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy fiscal data tool"
    ...
    echo "  Fiscal Data Tool: http://localhost:8364"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/${APP_NAME}-service 8364:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8364/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8364/api/capabilities | grep -q "capabilities"; then
        print_success "Capabilities endpoint working"
    else
        print_error "Capabilities endpoint not responding"
    fi

    # Test national debt capability
    echo ""
    print_info "Testing national debt capability..."
    curl -s -X POST http://localhost:8364/api/capabilities/national_debt \
        -H "Content-Type: application/json" \
        -d '{}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Test treasury rates capability
    echo ""
    print_info "Testing treasury rates capability..."
    curl -s -X POST http://localhost:8364/api/capabilities/treasury_rates \
        -H "Content-Type: application/json" \
        -d '{}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "fiscal-data-tool-service" 8364 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "fiscal-data-tool-service:8364:80" \
        "grafana:3000:80" \
...
cmd_status() {
    ...
    echo "Fiscal Data Tool Pod:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "Fiscal Data Tool Service:"
...
cmd_rollout() {
    ...
    # Load env to update config
    load_env

    # Update config from .env (no secrets needed - Treasury API is open)
...
cmd_clean() {
    ...
    print_info "Removing fiscal data tool deployment..."
...
cmd_rebuild() {
    ...
    # Setup config from .env (no secrets needed - Treasury API is open)
    setup_config
...
# Show help
cmd_help() {
    echo "Fiscal Data Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "NOTE: No API key required - Treasury Fiscal Data API is completely open."
    echo ""
    ...
    echo "  PORT              HTTP server port (default: 8364)"
    echo ""
    echo "Examples:"
