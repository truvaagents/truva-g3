#!/bin/bash
# Prometheus Query Tool Setup Script
# Provides commands for building, running, and deploying the prometheus query tool
...
APP_NAME="prometheus-query-tool"
PORT=${PORT:-8371}
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Prometheus Query Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Prometheus Query Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o prometheus-query-tool .

    print_success "Build completed: prometheus-query-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Prometheus Query Tool"
    ...
    print_info "Starting prometheus-query-tool on port $PORT..."
    print_info "Redis URL: $REDIS_URL"
    print_info "Prometheus URL: ${PROMETHEUS_URL:-http://prometheus.truvag3-examples:9090}"
    echo ""
...
setup_config() {
    truvag3_create_configmap "prometheus-query-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    # No secrets needed for Prometheus (no API key)
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy prometheus query tool"
    ...
    echo "  Prometheus Query Tool: http://localhost:8371"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/prometheus-query-tool-service $PORT:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3
    ...
    # Test query_metrics with PromQL
    echo ""
    print_info "Testing query_metrics (PromQL: up)..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/query_metrics \
        -H "Content-Type: application/json" \
        -d '{"query": "up"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Test get_targets
    echo ""
    print_info "Testing get_targets..."
    curl -s -X POST http://localhost:$PORT/api/capabilities/get_targets \
        -H "Content-Type: application/json" \
        -d '{"state": "active"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "prometheus-query-tool-service" 8371 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "prometheus-query-tool-service:8371:80" \
        "grafana:3000:80" \
...
# Show help
cmd_help() {
    echo "Prometheus Query Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the prometheus query tool service only"
    ...
    echo "  clean         Remove prometheus query tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8371)"
    echo "  PROMETHEUS_URL    Prometheus server URL (default: http://prometheus.truvag3-examples:9090)"
    echo ""
    echo "Configuration:"
