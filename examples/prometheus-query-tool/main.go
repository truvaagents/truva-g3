...
func main() {
	...
	tool := NewPrometheusTool()

	// Initialize telemetry AFTER tool creation
	initTelemetry("prometheus-query-tool")
	defer func() {
	...
	port := 8371 // default for prometheus-query-tool
	...
	framework, err := core.NewFramework(tool,
		...
		core.WithName("prometheus-query-tool"),
		core.WithPort(port),
		...
		core.WithMiddleware(telemetry.TracingMiddlewareWithConfig("prometheus-query-tool",
	...
	tool.Logger.Info("Prometheus Query Tool Service Starting", map[string]interface{}{
		"service":        "prometheus-query-tool",
		"port":           port,
		"prometheus_url": tool.prometheusURL,
		"telemetry":      "enabled",
		"operation":      "startup",
	})
...
func validateConfig() error {
	...
	if prometheusURL == "" {
		fmt.Fprintln(os.Stderr, "Info: PROMETHEUS_URL not set - defaulting to http://prometheus.truvag3-examples:9090")
	}
