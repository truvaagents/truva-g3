#!/bin/bash
# Economic Data Tool Setup Script
# Provides commands for building, running, and deploying the economic data tool
# NOTE: Requires FRED_API_KEY from https://fred.stlouisfed.org/ for live data (API calls fail without it)
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Economic Data Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Economic Data Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o economic-data-tool .

    print_success "Build completed: economic-data-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Economic Data Tool"
    ...
    # Check for FRED_API_KEY (required for live data - API calls fail without it)
    if [ -z "$FRED_API_KEY" ] || [ "$FRED_API_KEY" = "your_api_key_here" ]; then
        print_info "WARNING: FRED_API_KEY not set - API calls will return errors"
        echo ""
        echo "Get a FREE API key at: https://fred.stlouisfed.org/docs/api/api_key.html"
        echo "Then add to .env: FRED_API_KEY=your-key-here"
        echo ""
    else
        print_success "Using FRED_API_KEY from .env"
    fi

    # Build first
    cmd_build

    print_info "Starting economic-data-tool on port $PORT..."
...
# NOTE: This tool does NOT need AI API keys - it only needs FRED_API_KEY (optional)
# We intentionally do NOT create ai-provider-keys to avoid conflicts with agents
setup_api_keys() {
    truvag3_create_tool_secret "economic-data-tool-secrets" "$NAMESPACE" "FRED_API_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "economic-data-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    echo ""
    if [ -z "$FRED_API_KEY" ] || [ "$FRED_API_KEY" = "your_api_key_here" ]; then
        print_info "FRED_API_KEY not set - API calls will return errors. Add to .env and run './setup.sh rollout' to update."
    fi
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy economic data tool"
    ...
    echo "  Economic Data Tool: http://localhost:8363"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/economic-data-tool-service 8363:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8363/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8363/api/capabilities | grep -q "capabilities"; then
    ...
    echo ""
    print_info "Testing economic indicator lookup..."
    curl -s -X POST http://localhost:8363/api/capabilities/economic_indicator \
        -H "Content-Type: application/json" \
        -d '{"indicator":"mortgage_30y"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Test search indicators
    echo ""
    print_info "Testing indicator search..."
    curl -s -X POST http://localhost:8363/api/capabilities/search_indicators \
        -H "Content-Type: application/json" \
        -d '{"query":"unemployment"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "economic-data-tool-service" 8363 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "economic-data-tool-service:8363:80" \
        "grafana:3000:80" \
...
cmd_status() {
    ...
    echo "Economic Data Tool Pod:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "Economic Data Tool Service:"
...
cmd_clean() {
    ...
    print_info "Removing economic data tool deployment..."
...
# Show help
cmd_help() {
    echo "Economic Data Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "REQUIRED: FRED_API_KEY from https://fred.stlouisfed.org/"
    echo "  - API calls will fail without a key"
    echo "  - Get a FREE API key at: https://fred.stlouisfed.org/docs/api/api_key.html"
    echo "  - Add to .env file: FRED_API_KEY=your-key-here"
    echo ""
    echo "Local Development Commands:"
    echo "  build         Build the tool binary"
    echo "  run           Build and run the tool locally (requires FRED_API_KEY for API calls)"
    echo ""
    ...
    echo "  PORT              HTTP server port (default: 8363)"
    echo "  FRED_API_KEY      FRED API key (required for live data)"
