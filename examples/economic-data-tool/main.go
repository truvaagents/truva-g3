...
func main() {
	...
	initTelemetry("economic-data-tool")
	defer func() {
	...
	framework, err := core.NewFramework(tool,
		core.WithName("economic-data-tool"),
		core.WithPort(port),
		...
		core.WithMiddleware(telemetry.TracingMiddlewareWithConfig("economic-data-tool",
	...
	tool.Logger.Info("Economic data tool starting", map[string]interface{}{
		"operation":   "startup",
		"port":        port,
		"has_api_key": os.Getenv("FRED_API_KEY") != "",
	})

	if os.Getenv("FRED_API_KEY") == "" {
		tool.Logger.Warn("FRED_API_KEY not set - API calls will return errors. Get a free API key at: https://fred.stlouisfed.org/docs/api/api_key.html", map[string]interface{}{
			"operation": "startup",
		})
	}
	...
	go func() {
		<-sigChan
		tool.Logger.Info("Shutting down", map[string]interface{}{
			"operation": "shutdown",
		})
		cancel()
