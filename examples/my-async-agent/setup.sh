...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  My Async Agent - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
build() {
    ...
    GOWORK=off go build -o my-async-agent .

    print_success "Build completed: my-async-agent"
}
...
run() {
    ...
    print_info "Starting my-async-agent on port $PORT..."
...
setup_agent_config() {
    truvag3_create_configmap "my-async-agent-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
verify_ingress() {
    truvag3_verify_ingress \
        "my-async-agent.localhost" "alertmanager.localhost" \
...
mock_service() {
    ...
    case "$action" in
        ...
        degrade)
            ...
            print_info "Watch: Prometheus → AlertManager → my-async-agent → HITL gate"
            ;;
        recover)
...
deploy_split() {
    ...
    kubectl rollout restart deployment/my-async-agent-api -n $NAMESPACE
    print_info "Restarting Worker deployment..."
    kubectl rollout restart deployment/my-async-agent-worker -n $NAMESPACE

    # Wait for both
    print_info "Waiting for API deployment..."
    if kubectl rollout status deployment/my-async-agent-api -n $NAMESPACE --timeout=120s; then
        print_success "API pod ready"
    else
        print_error "API deployment failed"
        kubectl logs -n $NAMESPACE -l app=my-async-agent-api --tail=20
        exit 1
    fi

    print_info "Waiting for Worker deployment..."
    if kubectl rollout status deployment/my-async-agent-worker -n $NAMESPACE --timeout=120s; then
        print_success "Worker pod ready"
    else
        print_error "Worker deployment failed"
        kubectl logs -n $NAMESPACE -l app=my-async-agent-worker --tail=20
        exit 1
    fi
...
# Show help
show_help() {
    echo "My Async Agent Setup Script"
    echo ""
    ...
    echo "  PORT              HTTP server port (default: 8392)"
