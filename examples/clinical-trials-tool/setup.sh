#!/bin/bash
# Clinical Trials Tool Setup Script
# Provides commands for building, running, and deploying the clinical trials tool
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Clinical Trials Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Clinical Trials Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o clinical-trials-tool .

    print_success "Build completed: clinical-trials-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Clinical Trials Tool"
    ...
    print_info "Starting clinical-trials-tool on port $PORT..."
...
setup_config() {
    truvag3_create_configmap "clinical-trials-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    # No secrets needed - ClinicalTrials.gov is a public API
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy clinical trials tool"
    ...
    echo "  Clinical Trials Tool:  http://localhost:8367"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/clinical-trials-tool-service 8367:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8367/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8367/api/capabilities | grep -q "capabilities"; then
    ...
    # Test search_trials
    echo ""
    print_info "Testing search_trials..."
    curl -s -X POST http://localhost:8367/api/capabilities/search_trials \
        -H "Content-Type: application/json" \
        -d '{"condition": "lung cancer", "status": "RECRUITING", "max_results": 3}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "clinical-trials-tool-service" 8367 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "clinical-trials-tool-service:8367:80" \
        "grafana:3000:80" \
...
cmd_rollout() {
    ...
    # No secrets to update - ClinicalTrials.gov is a public API
...
# Show help
cmd_help() {
    echo "Clinical Trials Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the clinical trials tool service only"
    ...
    echo "  clean         Remove clinical trials tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8367)"
    echo ""
    echo "Note: ClinicalTrials.gov v2 is a public API - no API key required."
    echo ""
    echo "Configuration:"
