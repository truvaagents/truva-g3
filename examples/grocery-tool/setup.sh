...
setup_secrets() {
    truvag3_create_tool_secret "grocery-tool-config" "$NAMESPACE" "GROCERY_API_URL"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "grocery-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/grocery-tool-service 8336:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8336/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8336/api/capabilities | grep -q "capabilities"; then
    ...
    curl -s -X POST http://localhost:8336/api/capabilities/list_products \
...
cmd_forward() {
    truvag3_forward "grocery-tool-service" 8336 80
}

# Port forward for tool and monitoring
cmd_forward_all() {
    truvag3_forward_all \
        "grocery-tool-service:8336:80" \
        "grafana:3000:80" \
...
cmd_help() {
    ...
    echo "  PORT              HTTP server port (default: 8336)"
    echo ""
    echo "Examples:"
