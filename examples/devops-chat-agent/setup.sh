...
# setup.sh - One-click setup for devops-chat-agent
...
APP_NAME="devops-chat-agent"
AGENT_PORT=8357  # fallback port-forward only
...
print_header() {
    echo -e "${BLUE}╔═══════════════════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║     Truva-G3 DevOps Chat Agent                          ║${NC}"
    echo -e "${BLUE}╚═══════════════════════════════════════════════════════╝${NC}"
    echo ""
}
...
build_app() {
    log_info "Building devops-chat-agent..."
    ...
    # Build
    GOWORK=off go build -o devops-chat-agent .
...
build_docker() {
    ...
    truvag3_build_docker "devops-chat-agent:latest" \
        "$SCRIPT_DIR/Dockerfile.workspace" "$TRUVAG3_ROOT" $no_cache
}

load_to_kind() { truvag3_load_to_kind "devops-chat-agent:latest"; }

# Setup API keys as Kubernetes secrets
setup_k8s_secrets() {
    truvag3_create_secret "ai-provider-keys-devops-chat-agent" "$NAMESPACE"
}
...
deploy_k8s() {
    ...
    log_success "devops-chat-agent deployed"

    # Force rollout to pick up new images
    log_info "Rolling out new versions..."
    kubectl rollout restart deployment/devops-chat-agent -n $NAMESPACE
    kubectl rollout status deployment/devops-chat-agent -n $NAMESPACE --timeout=120s

    log_info "Waiting for pods to be ready..."
    kubectl wait --for=condition=ready pod -l app=devops-chat-agent -n $NAMESPACE --timeout=120s 2>/dev/null || true
...
print_summary() {
    ...
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo "Your DevOps Chat Agent is now running!"
    echo ""
    echo -e "${BLUE}Chat Application:${NC}"
    echo "  Chat API:   http://devops-chat-agent.localhost/health"
    echo "  Chat UI:    http://chat.localhost/devops.html"
    echo ""
    ...
    echo "  curl -X POST http://devops-chat-agent.localhost/chat/session | jq ."
    echo ""
    echo "  curl -N -X POST http://devops-chat-agent.localhost/chat/stream \"
    echo "    -H \"Content-Type: application/json\" \"
    echo "    -d '{\"message\": \"What is the cluster status?\"}'"
    echo ""
    echo -e "${BLUE}Useful commands:${NC}"
    echo "  kubectl get pods -n $NAMESPACE"
    echo "  kubectl logs -n $NAMESPACE -l app=devops-chat-agent -f"
...
# Test the API
test_api() {
    local host="${1:-devops-chat-agent.localhost}"

    log_info "Testing devops-chat-agent at $host..."
    echo ""
...
run_app() {
    log_info "Starting DevOps Chat Agent..."
    echo ""
...
full_deploy() {
    ...
    truvag3_verify_ingress "devops-chat-agent.localhost" "grafana.localhost" "jaeger.localhost" || true
    print_summary
}
...
rebuild() {
    ...
    if kubectl rollout status deployment/devops-chat-agent -n $NAMESPACE --timeout=120s; then
        log_success "devops-chat-agent rebuilt and deployed!"
    else
        log_error "Deployment failed. Checking logs..."
        kubectl logs -n $NAMESPACE -l app=devops-chat-agent --tail=20
        exit 1
    fi
}
...
# Show logs
logs() {
    log_info "Showing logs for devops-chat-agent..."
    kubectl logs -n "$NAMESPACE" -l app=devops-chat-agent -f
}
...
Examples:
  ...
  # DevOps queries to try:
  #   "What is the cluster status?"
  #   "List all pods in truvag3-examples namespace"
  #   "Show me the logs for the weather-tool pod"
  #   "Scale the currency-tool deployment to 3 replicas"
  #   "What time is it in UTC and Tokyo?"
EOF
}
...
case "${1:-help}" in
    ...
    full-deploy)
        ...
        ;;
    forward)
        truvag3_forward "devops-chat-agent-service" $AGENT_PORT 80
        ;;
    forward-all)
        truvag3_forward_all \
            "devops-chat-agent-service:$AGENT_PORT:80" \
            "grafana:3000:80" \
            "prometheus:9090:9090" \
            "jaeger-query:16686:80"
        ;;
    test)
        test_api "${2:-devops-chat-agent.localhost}"
        ;;
    logs)
        logs
        ;;
