#!/bin/bash
# Demographics Tool Setup Script
# Provides commands for building, running, and deploying the demographics tool
# NOTE: CENSUS_API_KEY from https://api.census.gov/data/key_signup.html is OPTIONAL
#       The tool works without it, but is limited to 500 requests/day
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Demographics Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Demographics Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o demographics-tool .

    print_success "Build completed: demographics-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Demographics Tool"
    ...
    # Check for CENSUS_API_KEY (optional - warn but don't exit)
    if [ -z "$CENSUS_API_KEY" ] || [ "$CENSUS_API_KEY" = "your_api_key_here" ]; then
        print_info "CENSUS_API_KEY not set - tool will work but is limited to 500 requests/day"
        echo ""
        echo "Get a FREE API key at: https://api.census.gov/data/key_signup.html"
        echo ""
        echo "Then add to .env: CENSUS_API_KEY=your-key-here"
        echo ""
    else
        print_success "Using CENSUS_API_KEY from .env"
    fi

    # Build first
    cmd_build

    print_info "Starting demographics-tool on port $PORT..."
...
# NOTE: This tool does NOT need AI API keys - it only needs CENSUS_API_KEY (optional)
# We intentionally do NOT create ai-provider-keys to avoid conflicts with agents
setup_api_keys() {
    truvag3_create_tool_secret "demographics-tool-secrets" "$NAMESPACE" "CENSUS_API_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "demographics-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    if [ -z "$CENSUS_API_KEY" ] || [ "$CENSUS_API_KEY" = "your_api_key_here" ]; then
        print_info "CENSUS_API_KEY not set - tool will work but is limited to 500 requests/day"
        print_info "Add CENSUS_API_KEY to .env and run './setup.sh rollout' to update"
    fi
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy demographics tool"
    ...
    echo "  Demographics Tool: http://localhost:8365"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/demographics-tool-service 8365:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8365/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8365/api/capabilities | grep -q "capabilities"; then
        print_success "Capabilities endpoint working"
    else
        print_error "Capabilities endpoint not responding"
    fi

    # Test area statistics capability
    echo ""
    print_info "Testing area statistics..."
    curl -s -X POST http://localhost:8365/api/capabilities/area_statistics \
        -H "Content-Type: application/json" \
        -d '{"location": "New York"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "demographics-tool-service" 8365 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "demographics-tool-service:8365:80" \
        "grafana:3000:80" \
...
cmd_status() {
    ...
    echo "Demographics Tool Pod:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "Demographics Tool Service:"
...
cmd_clean() {
    ...
    print_info "Removing demographics tool deployment..."
...
# Show help
cmd_help() {
    echo "Demographics Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "NOTE: CENSUS_API_KEY from https://api.census.gov/data/key_signup.html is OPTIONAL"
    echo "  - The tool works without it, but is limited to 500 requests/day"
    echo "  - Add to .env file: CENSUS_API_KEY=your-key-here"
    echo ""
    ...
    echo "  PORT              HTTP server port (default: 8365)"
    echo "  CENSUS_API_KEY    Census Bureau API key (OPTIONAL - 500 req/day without key)"
