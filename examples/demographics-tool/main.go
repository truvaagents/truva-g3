...
func main() {
	...
	tool := NewDemographicsTool()

	initTelemetry("demographics-tool")
	defer func() {
	...
	if tool.apiKey == "" {
		log.Println("Warning: CENSUS_API_KEY not set. API limited to 500 requests/day. Get a free key at https://api.census.gov/data/key_signup.html")
	}
	...
	framework, err := core.NewFramework(tool,
		core.WithName("demographics-tool"),
		core.WithPort(port),
		...
		core.WithMiddleware(telemetry.TracingMiddlewareWithConfig("demographics-tool",
