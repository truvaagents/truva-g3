#!/bin/bash
# Web Search Tool Setup Script
# Provides commands for building, running, and deploying the web search tool
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Web Search Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
setup_api_keys() {
    truvag3_create_tool_secret "web-search-secrets" "$NAMESPACE" "SEARCH_API_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "web-search-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
# Build the tool
cmd_build() {
    print_header "Building Web Search Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o web-search-tool .

    print_success "Build completed: web-search-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Web Search Tool"
    ...
    print_info "Starting web-search-tool on port $PORT..."
...
cmd_full_deploy() {
    ...
    echo "  Web Search Tool: http://localhost:8341"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/web-search-tool-service 8341:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8341/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8341/api/capabilities | grep -q "capabilities"; then
    ...
    curl -s -X POST http://localhost:8341/api/capabilities/web_search \
        -H "Content-Type: application/json" \
        -d '{"query": "best beach destinations"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Kill port forward
    kill $PF_PID 2>/dev/null || true
}

# Port forward for local access (background)
cmd_forward() {
    truvag3_forward "web-search-tool-service" 8341 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "web-search-tool-service:8341:80" \
        "grafana:3000:80" \
...
# Show help
cmd_help() {
    echo "Web Search Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the web search service only"
    ...
    echo "  clean         Remove web search tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8341)"
    echo "  SEARCH_API_KEY    Tavily API key for real web search"
    echo "                    Get a FREE key: https://www.tavily.com"
    echo ""
    echo "Configuration:"
