#!/bin/bash
# OpenFDA Tool Setup Script
# Provides commands for building, running, and deploying the openfda tool
...
# Build the tool
cmd_build() {
    print_header "Building OpenFDA Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o openfda-tool .

    print_success "Build completed: openfda-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running OpenFDA Tool"
    ...
    print_info "Starting openfda-tool on port $PORT..."
...
setup_config() {
    truvag3_create_configmap "openfda-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    truvag3_create_tool_secret "openfda-tool-secrets" "$NAMESPACE" "OPENFDA_API_KEY"
    setup_config
...
cmd_full_deploy() {
    ...
    echo "  OpenFDA Tool: http://localhost:8374"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/openfda-tool-service 8374:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8374/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8374/api/capabilities | grep -q "capabilities"; then
    ...
    echo ""
    print_info "Testing adverse events search..."
    curl -s -X POST http://localhost:8374/api/capabilities/search_adverse_events \
        -H "Content-Type: application/json" \
        -d '{"drug_name": "aspirin"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "openfda-tool-service" 8374 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "openfda-tool-service:8374:80" \
        "grafana:3000:80" \
...
# Show help
cmd_help() {
    echo "OpenFDA Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the OpenFDA tool service only"
    ...
    echo "  clean         Remove OpenFDA tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8374)"
    echo "  OPENFDA_API_KEY   OpenFDA API key (optional, increases rate limits)"
    echo "                    Register at: https://open.fda.gov/apis/authentication/"
    echo ""
    echo "Configuration:"
