#!/bin/bash
# JIRA Tool Setup Script
# Provides commands for building, running, and deploying the JIRA issue management tool
...
# Build the tool
cmd_build() {
    print_header "Building JIRA Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o jira-tool .

    print_success "Build completed: jira-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running JIRA Tool"
    ...
    if [ -z "$JIRA_BASE_URL" ]; then
        print_error "JIRA_BASE_URL environment variable is required"
        print_info "Set it in .env file (e.g. https://mycompany.atlassian.net)"
        exit 1
    fi
    ...
    print_info "Starting jira-tool on port $PORT..."
    print_info "Redis URL: $REDIS_URL"
    print_info "JIRA Base URL: $JIRA_BASE_URL"
    echo ""
...
setup_api_keys() {
    ...
    truvag3_create_tool_secret "jira-tool-secrets" "$NAMESPACE" "JIRA_BASE_URL" "JIRA_USER_EMAIL" "JIRA_API_TOKEN"
}

# Setup config as Kubernetes ConfigMap
setup_config() {
    truvag3_create_configmap "jira-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/jira-tool-service 8366:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8366/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8366/api/capabilities | grep -q "capabilities"; then
    ...
    # Test get_issue
    echo ""
    print_info "Testing get_issue..."
    curl -s -X POST http://localhost:8366/api/capabilities/get_issue \
        -H "Content-Type: application/json" \
        -d '{"issue_key": "TRUV-1"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"

    # Test search_issues
    echo ""
    print_info "Testing search_issues..."
    curl -s -X POST http://localhost:8366/api/capabilities/search_issues \
        -H "Content-Type: application/json" \
        -d '{"jql": "project = TRUV", "max_results": 5}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "jira-tool-service" 8366 80
}

# Port forward for tool and monitoring
cmd_forward_all() {
    truvag3_forward_all \
        "jira-tool-service:8366:80" \
        "grafana:3000:80" \
...
# Clean up tool only
cmd_clean() {
    print_header "Cleaning Up JIRA Tool"

    print_info "Removing tool deployment..."
    kubectl delete -f k8-deployment.yaml --ignore-not-found
    kubectl delete secret jira-tool-secrets -n $NAMESPACE --ignore-not-found
    print_success "JIRA tool cleanup complete"
}
...
cmd_clean_all() {
    ...
    kubectl delete secret jira-tool-secrets -n $NAMESPACE --ignore-not-found 2>/dev/null || true
...
# Show help
cmd_help() {
    echo "JIRA Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "Local Development Commands:"
    echo "  build         Build the jira-tool binary"
    ...
    echo "  JIRA_BASE_URL     JIRA Cloud URL (required, e.g. https://mycompany.atlassian.net)"
    echo "  JIRA_USER_EMAIL   Atlassian account email (required)"
    echo "  JIRA_API_TOKEN    JIRA API token (required)"
    echo "  REDIS_URL         Redis connection URL (required for run)"
    echo "  PORT              HTTP server port (default: 8366)"
    echo ""
    echo "Examples:"
    ...
    echo "  ./setup.sh test           # Run JIRA tool tests"
