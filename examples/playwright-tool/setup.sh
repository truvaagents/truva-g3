#!/bin/bash
# Playwright Tool Setup Script
# Provides commands for building, running, and deploying the playwright tool
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Playwright Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Playwright Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o playwright-tool .

    print_success "Build completed: playwright-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Playwright Tool"
    ...
    print_info "Starting playwright-tool on port $PORT..."
...
_create_s3_secret() {
    truvag3_create_tool_secret "playwright-s3-credentials" "$NAMESPACE" "S3_ACCESS_KEY" "S3_SECRET_KEY"
}

# Setup config as Kubernetes ConfigMap (captures TRUVAG3_*, APP_ENV, DEV_MODE, + tool-specific vars)
setup_config() {
    truvag3_create_configmap "playwright-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_deploy() {
    ...
    # Setup S3 credentials as secret
    _create_s3_secret
    setup_config
    ...
    if kubectl wait --for=condition=available --timeout=180s deployment/$APP_NAME -n $NAMESPACE 2>/dev/null; then
...
cmd_full_deploy() {
    ...
    echo "  Playwright Tool: http://localhost:8349"
...
cmd_test() {
    ...
    if curl -s http://localhost:8349/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8349/api/capabilities | jq -e 'type == "array" and length > 0' >/dev/null 2>&1; then
    ...
    # Test explore_page
    echo ""
    print_info "Testing explore_page capability..."
    curl -s -X POST http://localhost:8349/api/capabilities/explore_page \
        -H "Content-Type: application/json" \
        -d '{"url": "https://example.com"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
# Show help
cmd_help() {
    echo "Playwright Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the playwright tool service only"
    ...
    echo "  clean         Remove playwright tool deployment only"
    ...
    echo "  PORT              HTTP server port (default: 8349)"
    echo "  S3_ENDPOINT       S3/MinIO endpoint URL (leave empty for real AWS)"
    ...
    echo "  ./setup.sh rebuild                  # Fresh build and deploy"
    echo "  ./setup.sh forward                  # Port forward tool only"
