...
setup_config() {
    truvag3_create_configmap "stock-market-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}
...
cmd_full_deploy() {
    ...
    echo "  Stock Tool:  http://localhost:8338"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/stock-tool-service 8338:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8338/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8338/api/capabilities | grep -q "capabilities"; then
    ...
    curl -s -X POST http://localhost:8338/api/capabilities/stock_quote \
...
cmd_forward() {
    truvag3_forward "stock-tool-service" 8338 80
}

# Port forward all services (tool + monitoring)
cmd_forward_all() {
    truvag3_forward_all \
        "stock-tool-service:8338:80" \
        "grafana:3000:80" \
...
cmd_help() {
    ...
    echo "  PORT              HTTP server port (default: 8338)"
