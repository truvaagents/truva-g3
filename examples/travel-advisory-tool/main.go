...
func main() {
	...
	tool := NewAdvisoryTool()

	initTelemetry("travel-advisory-tool")
	defer func() {
	...
	framework, err := core.NewFramework(tool,
		core.WithName("travel-advisory-tool"),
		core.WithPort(port),
		...
		core.WithMiddleware(telemetry.TracingMiddlewareWithConfig("travel-advisory-tool",
	...
	tool.Logger.Info("Travel Advisory Tool Service Starting", map[string]interface{}{
		"service": "travel-advisory-tool", "port": port, "telemetry": "enabled",
	})
	...
	go func() {
		<-sigChan
		tool.Logger.Info("Shutting down gracefully", map[string]interface{}{"service": "travel-advisory-tool"})
		cancel()
...
func validateConfig() error {
	...
	// No API key required - State Dept API is free and open
	return nil
}
