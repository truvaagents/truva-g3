#!/bin/bash
# World Health Tool Setup Script
# Provides commands for building, running, and deploying the world-health-tool
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  World Health Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building World Health Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o world-health-tool .

    print_success "Build completed: world-health-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running World Health Tool"
    ...
    print_info "No API key needed -- WHO GHO and World Bank APIs are free"

    # Build first
    cmd_build

    print_info "Starting world-health-tool on port $PORT..."
...
setup_config() {
    truvag3_create_configmap "world-health-tool-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"
}

# Deploy to Kubernetes
# NOTE: No secrets needed -- WHO GHO and World Bank APIs are free
cmd_deploy() {
    ...
    echo ""
    print_info "No API keys needed -- WHO GHO and World Bank APIs are free"
...
cmd_full_deploy() {
    ...
    echo "  3. Deploy world health tool"
    ...
    echo "  World Health Tool: http://localhost:8368"
...
cmd_test() {
    ...
    if curl -s http://localhost:8368/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8368/api/capabilities | grep -q "capabilities"; then
        print_success "Capabilities endpoint working"
    else
        print_error "Capabilities endpoint not responding"
    fi

    # Test get_health_indicator
    echo ""
    print_info "Testing health indicator lookup..."
    curl -s -X POST http://localhost:8368/api/capabilities/get_health_indicator \
        -H "Content-Type: application/json" \
        -d '{"indicator":"life_expectancy","country":"USA"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_status() {
    ...
    echo "World Health Tool Pod:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "World Health Tool Service:"
...
cmd_clean() {
    ...
    print_info "Removing world health tool deployment..."
...
# Show help
cmd_help() {
    echo "World Health Tool Setup Script"
    echo ""
    echo "Usage: ./setup.sh <command>"
    echo ""
    echo "NOTE: No API keys needed -- WHO GHO and World Bank APIs are free"
    echo ""
    ...
    echo "  PORT              HTTP server port (default: 8368)"
