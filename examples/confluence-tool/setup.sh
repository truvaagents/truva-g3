#!/bin/bash
# Confluence Tool Setup Script
# Provides commands for building, running, and deploying the Confluence tool
...
print_header() {
    echo -e "${BLUE}================================================${NC}"
    echo -e "${BLUE}  Confluence Tool - $1${NC}"
    echo -e "${BLUE}================================================${NC}"
}
...
# Build the tool
cmd_build() {
    print_header "Building Confluence Tool"

    print_info "Running go mod tidy..."
    GOWORK=off go mod tidy

    print_info "Building binary..."
    GOWORK=off go build -o confluence-tool .

    print_success "Build completed: confluence-tool"
}

# Run the tool locally
cmd_run() {
    print_header "Running Confluence Tool"
    ...
    if [ -z "$CONFLUENCE_BASE_URL" ] || [ -z "$CONFLUENCE_USER_EMAIL" ] || [ -z "$CONFLUENCE_API_TOKEN" ]; then
        print_error "Confluence credentials are required"
        print_info "Set CONFLUENCE_BASE_URL, CONFLUENCE_USER_EMAIL, and CONFLUENCE_API_TOKEN in .env"
    ...
    print_info "Starting confluence-tool on port $PORT..."
    print_info "Redis URL: $REDIS_URL"
    print_info "Confluence: $CONFLUENCE_BASE_URL"
    echo ""
...
cmd_deploy() {
    ...
    # Setup ConfigMap from .env (user-configurable vars like APP_ENV, DEV_MODE, TRUVAG3_*)
    truvag3_create_configmap "${APP_NAME}-env-config" "$NAMESPACE" "$SCRIPT_DIR/.env"

    # Setup API keys as secrets (3 keys for Atlassian auth)
    truvag3_create_tool_secret "confluence-tool-secrets" "$NAMESPACE" "CONFLUENCE_BASE_URL" "CONFLUENCE_USER_EMAIL" "CONFLUENCE_API_TOKEN"
...
cmd_full_deploy() {
    ...
    echo "  Confluence Tool: http://localhost:8376"
...
cmd_test() {
    ...
    kubectl port-forward -n $NAMESPACE svc/confluence-tool-service 8376:80 >/dev/null 2>&1 &
    PF_PID=$!
    sleep 3

    # Test health endpoint
    echo "Testing health endpoint..."
    if curl -s http://localhost:8376/health | grep -q "healthy"; then
    ...
    if curl -s http://localhost:8376/api/capabilities | grep -q "capabilities"; then
    ...
    # Test search
    echo ""
    print_info "Testing search_pages..."
    curl -s -X POST http://localhost:8376/api/capabilities/search_pages \
        -H "Content-Type: application/json" \
        -d '{"query": "test"}' | jq . 2>/dev/null || echo "(install jq for pretty output)"
...
cmd_forward() {
    truvag3_forward "confluence-tool-service" 8376 80
}

# Port forward all services
cmd_forward_all() {
    truvag3_forward_all \
        "confluence-tool-service:8376:80" \
        "grafana:3000:80" \
...
cmd_status() {
    ...
    echo "Confluence Tool Pods:"
    kubectl get pods -n $NAMESPACE -l app=$APP_NAME
    echo ""
    echo "Confluence Tool Service:"
...
cmd_clean() {
    ...
    kubectl delete secret confluence-tool-secrets -n $NAMESPACE --ignore-not-found
...
cmd_clean_all() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        kubectl delete secret confluence-tool-secrets -n $NAMESPACE --ignore-not-found 2>/dev/null || true
...
# Show help
cmd_help() {
    echo "Confluence Tool Setup Script"
    echo ""
    ...
    echo "  forward       Port forward the confluence tool service only"
    ...
    echo "  clean         Remove confluence tool deployment only"
    ...
    echo "  PORT                    HTTP server port (default: 8376)"
    echo "  CONFLUENCE_BASE_URL     Atlassian instance URL (required)"
    echo "  CONFLUENCE_USER_EMAIL   Atlassian account email (required)"
    echo "  CONFLUENCE_API_TOKEN    Atlassian API token (required)"
    echo "                          Same token as JIRA - generate at:"
