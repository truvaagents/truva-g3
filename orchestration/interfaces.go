...
type OrchestratorConfig struct {
	...
	// PlanRefinementEnabled enables the post-orchestrator plan-refinement pass.
	// It is resolved at construction so request execution never reads process
	// environment. Env: TRUVAG3_PLAN_REFINEMENT_ENABLED.
	PlanRefinementEnabled bool `json:"plan_refinement_enabled"`
	...
	// Canonical construction uses NoOp when nil. Compatibility constructors
	// retain the historical Redis-from-environment adapter.
	...
	// Executor retry backoff is private framework wiring because the public
	// compatibility surface historically exposed only environment variables
	// and setter methods for these values. Code-owned configuration uses
	// WithStepRetryBackoff; the canonical resolver owns environment parsing.
	stepRetryBackoff core.BackoffConfig

	// executionStoreWriteTimeout bounds each asynchronous diagnostic-store write.
	// Code overrides use WithExecutionStoreWriteTimeout; deployment overrides use
	// TRUVAG3_EXECUTION_STORE_WRITE_TIMEOUT.
	executionStoreWriteTimeout time.Duration
}
