...
- `/api/capabilities/{name}/schema` — the auto-generated JSON Schema endpoint the framework exposes per capability when `InputSummary` is provided, used by the orchestration module's schema-cache validation layer (see [TOOL_SCHEMA_DISCOVERY_GUIDE.md](TOOL_SCHEMA_DISCOVERY_GUIDE.md) Phase 3)
...
Quality of the spec is entirely a function of the quality of the `InputSummary` and `OutputSummary` you provide when registering capabilities. See [TOOL_DEVELOPMENT_GUIDE.md](TOOL_DEVELOPMENT_GUIDE.md) Section 5 and [TOOL_SCHEMA_DISCOVERY_GUIDE.md](TOOL_SCHEMA_DISCOVERY_GUIDE.md) for the authoring conventions.
...
| Redis HITL checkpoint store (default prefix `truvag3:hitl:checkpoint:*`, configurable via `TRUVAG3_HITL_KEY_PREFIX`) | Pending and expired checkpoints with plan, current step, resolved parameters, decision, status, agent name, agent address, request mode | HITL Interrupted view; DAG detail panel's HITL tab |
...
| HITL Interrupted | At least one agent must run the orchestration module with HITL enabled (`HITLConfig.Enabled: true` and a configured checkpoint store). See [HUMAN_IN_THE_LOOP_USER_GUIDE.md](HUMAN_IN_THE_LOOP_USER_GUIDE.md). |
...
| Memory | Agents must be wired with shared memory via `memory.NewSharedBackends(...)` and `orchestration.BuildMemoryHooks(...)`. See [AGENT_MEMORY_USER_GUIDE.md](AGENT_MEMORY_USER_GUIDE.md). `TRUVAG3_AGENT_DOMAIN` must be set so the view has a domain to pick from in its dropdown. |
...
**Related guides:**
- **[Tool Development Guide](TOOL_DEVELOPMENT_GUIDE.md)** — How to write good `InputSummary` and `OutputSummary` when registering capabilities. Spec quality depends on this.
